...
public:
    ...
    void print_metrics() const {
        ...
        std::cout << "\n=============================================\n";
        std::cout << "   PERFORMANCE METRICS & STATISTICS (VAR 17)\n";
        std::cout << "=============================================\n";

        // 1. Total created threads
        std::cout << "* Active worker threads created: 6\n";

        // 2. Average thread wait duration
        std::cout << "* Worker threads idle/waiting time profiles:\n";
        ...
        for (size_t i = 0; i < 6; ++i) {
            ...
            std::cout << "  - Worker Thread #" << i << ": " << avg_wait << " ms (wait events: " << m_worker_wait_counts[i] << ")\n";
            total_avg += avg_wait;
        }
        std::cout << "  -> Total pool average idle wait time: " << (total_avg / 6.0) << " ms\n";

        // 3. Queue full saturation duration
        long long min_full_time = 0;
        long long max_full_time = 0;
        m_tasks.get_fullness_metrics(min_full_time, max_full_time);
        std::cout << "* Queue full saturation duration (Limit: 20 tasks):\n";
        std::cout << "  - Minimum continuous full time: " << min_full_time << " ms\n";
        std::cout << "  - Maximum continuous full time: " << max_full_time << " ms\n";

        // 4. Rejected tasks count
        std::cout << "* Total tasks rejected due to queue overflow: " << m_rejected_tasks_count << "\n";
        std::cout << "=============================================\n\n";
    }
